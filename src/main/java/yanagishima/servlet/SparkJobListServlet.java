...
public class SparkJobListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<SparkSqlJob> notRunningList = sparkSqlJobList.stream().filter(j -> j.getJobIds().size() > 0)
                .filter(j -> j.getState().equals("FINISHED") || j.getState().equals("FAILED"))
                .sorted((a, b) -> b.getStartTime().compareTo(a.getStartTime())).limit(LIMIT)
